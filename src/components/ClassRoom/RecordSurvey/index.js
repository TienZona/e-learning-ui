...
function RecordSurvey({ meet }) {
    const [surveys, setSurveys] = useState([]);

    useEffect(() => {
        axios
            .get(`http://localhost:3000/meet/survey/${meet.id_room}`)
            .then((res) => {
                if (res.status === 200) {
                    setSurveys(res.data);
                }
                    ...
                    {surveys.map((survey, index) => (
                        <div key={index}>
                            <div className="flex items-center">
                                ...
                                <h1 className="px-3 text-3xl">{survey.heading}</h1>
                            </div>
                            {survey.answers.map((answer, index) => (
                                <div className="mx-16 my-3" key={index}>
                                    <div className="flex justify-between my-3">
                                        <h2 className="text-2xl">{index + 1 + '. ' + answer.content}</h2>
                                        <h2 className="text-2xl">{answer.vote.length} bình chọn </h2>
                                    </div>
